/* 
 ...
 DebugVersion_12_BestDebugger, 1stAugust 2014
...
word    sDelay     = 250;  //Small Delay about 1/4 of bit duration  try like 230 to 500
...
void setup() {
  ...
  Serial.println("Debug Manchester Version 12 Best Debug- Dumps Raw bit pattern of different length packets");
  ...
  Serial.println("D 00 00001111 01 22223333 02 44445555 03 66667777 04 88889999 05 AAAABBBB 06 CCCCDDDD 07 EEEEFFFF 08 00001111 90 22223333 A0 44445555"); 
  eraseManchester();
}
...
void add(byte bitData){
  if (discNos>0){ 
    ...
    if(manchester[0]==0x54){
      maxBytes=11;//rain
    }
    if(manchester[0]==0x58){
      maxBytes=10;//wind
    }
    if(manchester[0]==0x5F){
      maxBytes=9;//temp
    }
    if(manchester[0]==0x5C){
      maxBytes=10;//Experimental
    }
  }
}
